...
public class UserListServlet extends HttpServlet {
    ...
    private static final String QUERY = "SELECT id AS Идентификатор, user_name AS"
            + " Имя, email AS Email, age AS Возраст FROM user_data ORDER BY id ASC;";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try(Connection connection = DriverManager.getConnection("jdbc:mysql:///book_app", "root", "masterkey");
                ...
                out.println("</head>");
                out.println("<body>");
                out.println("<h1>СПИСОК ЗАРЕГИСТРИРОВАННЫХ ПОЛЬЗОВАТЕЛЕЙ</h1><br>");
                ...
                while(rs.next()) {
                    ...
                    out.println("<td style='padding-left: 5px;"
                            + "padding-right: 5px;border: 1px solid #bc0505;'>"
                                    + "<a href='userScreen?id="
                            + rs.getInt(1) + "'>Редактировать</a></td>");
                    out.println("<td style='padding-left: 5px;"
                            + "padding-right: 5px;border: 1px solid #bc0505;'>"
                                    + "<a href='deleteUserUrl?id="
